! function() {
    ...
    function n() {
        "ontouchstart" in window || window.addEventListener("mousemove", t), window.addEventListener("scroll", o), window.addEventListener("resize", i)
    }
    ...
    function i() {
        u = window.innerWidth, v = window.innerHeight, h.width = u, h.height = v
    }
    ...
    function r() {
        if (y) {
            m.clearRect(0, 0, u, v);
            for (var e in w) Math.abs(s(g, w[e])) < 4e3 ? (w[e].active = .3, w[e].circle.active = .6) : Math.abs(s(g, w[e])) < 2e4 ? (w[e].active = .1, w[e].circle.active = .3) : Math.abs(s(g, w[e])) < 4e4 ? (w[e].active = .02, w[e].circle.active = .1) : (w[e].active = 0, w[e].circle.active = 0), l(w[e]), w[e].circle.draw()
        }
    ...
    function l(e) {
        if (e.active)
            for (var n in e.closest) m.beginPath(), m.moveTo(e.x, e.y), m.lineTo(e.closest[n].x, e.closest[n].y), m.strokeStyle = "rgba(255,255,255," + e.active + ")", m.stroke()
    }

    function d(e, n, t) {
        var o = this;
        ! function() {
            o.pos = e || null, o.radius = n || null, o.color = t || null
        }(), this.draw = function() {
